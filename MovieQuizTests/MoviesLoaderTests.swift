...
class MoviesLoaderTests: XCTestCase {
    func testSuccessLoading() throws {
        //Given
        let stubNetworkClient = StubNetworkClient(emulateError: false) //говорим что не хотим эулировать ошибку
        let loader = MoviesLoader(networkClient: stubNetworkClient)
        
        //When
        //Так как функция загрузки фильмов - ассинхронная, нужно ожидание
        ...
        loader.loadMovies { result in
            ...
            case .success(let movies):
                ...
                XCTAssertEqual(movies.items.count, 2) // провееряем что пришло 2 фильма, в тест данных их 2
                expectation.fulfill()
            case .failure(_):
                //Мы не ожидаем, что пришла ошибка, если она появиться надо провалить тест
    ...
    func testFailureLoading() throws {
        //Given
        let stubNetworkClient = StubNetworkClient(emulateError: true) // говорим что нужна ошибка
        let loader = MoviesLoader(networkClient: stubNetworkClient)
